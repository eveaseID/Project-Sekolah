@if ($componentType == "ADD")
    @include('livewire.paketwisata.createPaketWisata')
@endif

@if ($componentType == "EDIT")
    @include('livewire.paketwisata.editPaketWisata')
@endif

@if ($componentType == "DETAIL")
    @include('livewire.paketwisata.detailPaketWisata')
@endif


@if ($componentType == "LIST")

@section('title')
    Paket Wisata
@endsection

<div>
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="text-header mb-5">
                <h4 class="text-primary">
                    Table Paket Wisata 
                    <button wire:click="addForm" class="btn btn-primary float-end">Add <i class='bx bx-plus'></i></button>
                    <input class="form-control float-end mx-2" type="text" style="width: 300px" wire:model="search" placeholder="Search...">
                </h4>
            </div>
            
            <div class="shadow-sm rounded bg-white p-4">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama Paket</th>
                            <th>Deskripsi</th>
                            <th>Fasilitas</th>
                            <th>Itinerary</th>
                            <th>Diskon</th>
                            <th>foto1</th>
                            <th>foto2</th>
                            <th>foto3</th>
                            <th>foto4</th>
                            <th>foto5</th>
                            <th class="text-center">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($paketWisata as $key => $item)
                            <tr>
                                <td>{{ $paketWisata->firstItem() + $key }}</td>
                                <td>{{ $item->nama_paket }}</td>
                                <td>{{ \Illuminate\Support\Str::limit($item->deskripsi, 8, '...') }}</td>
                                <td>{{ \Illuminate\Support\Str::limit($item->fasilitas, 8, '...') }}</td>
                                <td>{{ Illuminate\Support\Str::limit($item->itinerary, 8, '...') }}</td>
                                <td>{!! number_format($item->diskon) !!}%</td>
                                <td>
                                    <img src="{{ asset('uploads/paketwisata_images') }}/{{ $item->foto1 }}" style="width: 50px; height: 50px; object-fit: cover">
                                </td>
                                <td>
                                    <img src="{{ asset('uploads/paketwisata_images') }}/{{ $item->foto2 }}" style="width: 50px; height: 50px; object-fit: cover">
                                </td>
                                <td>
                                    <img src="{{ asset('uploads/paketwisata_images') }}/{{ $item->foto3 }}" style="width: 50px; height: 50px; object-fit: cover">
                                </td>
                                <td>
                                    <img src="{{ asset('uploads/paketwisata_images') }}/{{ $item->foto4 }}" style="width: 50px; height: 50px; object-fit: cover">
                                </td>
                                <td>
                                    <img src="{{ asset('uploads/paketwisata_images') }}/{{ $item->foto5 }}" style="width: 50px; height: 50px; object-fit: cover">
                                </td>

                                <td class="text-center">
                                    <button wire:click="detailForm({{ $item->id }})" class="btn btn-primary">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(255, 255, 255, 1);transform: ;msFilter:;"><path d="M20 3H4c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zM4 19V5h16l.002 14H4z"></path><path d="M6 7h12v2H6zm0 4h12v2H6zm0 4h6v2H6z"></path></svg>
                                    </button>

                                    <button wire:click="editForm({{ $item->id }})" class="btn btn-warning">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgb(255, 255, 255);transform: ;msFilter:;"><path d="m18.988 2.012 3 3L19.701 7.3l-3-3zM8 16h3l7.287-7.287-3-3L8 13z"></path><path d="M19 19H8.158c-.026 0-.053.01-.079.01-.033 0-.066-.009-.1-.01H5V5h6.847l2-2H5c-1.103 0-2 .896-2 2v14c0 1.104.897 2 2 2h14a2 2 0 0 0 2-2v-8.668l-2 2V19z"></path></svg>
                                    </button>

                                    <button wire:click="deletePaketWisata({{ $item->id }})" class="btn btn-danger">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgb(255, 255, 255);transform: ;msFilter:;"><path d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm10.618-3L15 2H9L7.382 4H3v2h18V4z"></path></svg>
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <td colspan="12" class="text-center">Record Not Found</td>    
                        @endforelse
                    </tbody>
                </table>
                {{ $paketWisata->links() }}
            </div>
        </div>
    </div>
</div>
@endif

<script>
    window.addEventListener('showSuccess', event => {
        defaultToast({title: event.detail.title})
    })

    window.addEventListener('showDeleteAlert', event => {
        Swal.fire({
        title: 'Are you sure?',
        text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, delete it!'
        }).then((result) => {
        if (result.isConfirmed) {
            livewire.emit('removePaketWisata')
            Swal.fire(
            'Deleted!',
            'Your file has been deleted.',
            'success'
            )
        }
        })
    })
</script>